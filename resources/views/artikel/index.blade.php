            ...
            <section class="bg-gray-50 dark:bg-gray-900">
                <div class="mx-auto max-w-screen-xl">
                    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-4">
                        <h1 class="text-sm font-medium dark:text-white">Artikel</h1>
                    </div>
                </div>
            </section>
        </div>
                ...
                <div class="mx-auto max-w-screen-xl">
                    <div
                        ...
                        <div
                            ...
                            @if (Auth::user()->level == 'ADMIN' || Auth::user()->level == 'PETUGAS')
                                <a href="{{ url('/artikel/tambah') }}" type="button"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 mb-4">
                        @foreach ($artikel as $a)
                            <div
                                class="h-full w-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                                <a>
                                    <img class="rounded-t-lg w-full h-60 object-cover"
                                        src="{{ asset('storage/' . $a->gambar) }}" alt="" />
                                </a>
                                <div class="p-5">
                                    <a>
                                    ...
                                    <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">
                                        {{ Str::limit($a->isi, 200) }}</p>
                                    <div class="flex items-center justify-between space-x-4">
                                        <a href="{{ url('/artikel/detail/' . $a->id) }}"
                                        ...
                                        @if (Auth::user()->level == 'ADMIN' || Auth::user()->level == 'PETUGAS')
                                            <button type="button" data-modal-target="deleteModal"
                                                data-modal-toggle="deleteModal" data-id="{{ $a->id }}"
    ...
    <script>
        document.querySelectorAll('[data-modal-toggle="deleteModal"]').forEach(link => {
            link.addEventListener('click', (event) => {
                ...
                confirmDeleteForm.action = `/artikel/hapus/${id}`;
            });
        });
    </script>
@endsection
