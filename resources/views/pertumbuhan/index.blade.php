            ...
            <section class="bg-gray-50 dark:bg-gray-900">
                <div class="mx-auto max-w-screen-xl">
                    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-4">
                        <h1 class="text-sm font-medium dark:text-white">Pertumbuhan</h1>
                    </div>
                </div>
            </section>
        </div>
            ...
            <section class="bg-gray-50 dark:bg-gray-900">
                <div class="mx-auto max-w-screen-xl">
                    ...
                    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                        <div
                            ...
                            <div
                                ...
                                @if (Auth::user()->level == 'ADMIN' || Auth::user()->level == 'PETUGAS')
                                    <a href="{{ url('/pertumbuhan/tambah') }}" type="button"
                            ...
                            <table id="dataTable" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3">Tanggal Posyandu</th>
                                        @if (Auth::user()->level == 'ADMIN' || Auth::user()->level == 'PETUGAS')
                                ...
                                <tbody>
                                    @foreach ($pertumbuhan as $p)
                                        <tr class="border-b dark:border-gray-700">
                                            ...
                                            <td class="px-4 py-3">{{ $p->berat_badan }}</td>
                                            <td class="px-4 py-3">{{ $p->tinggi_badan }}</td>
                                            <td class="px-4 py-3">{{ $p->umur }}</td>
                                            <td class="px-4 py-3 format-tanggal">{{ $p->tanggal_posyandu }}</td>
                                            @if (Auth::user()->level == 'ADMIN' || Auth::user()->level == 'PETUGAS')
                                                <td class="px-4 py-3 flex items-center justify-end">
                                                    <button id="apple-imac-27-dropdown-button"
                                                        data-dropdown-toggle="{{ $p->id }}"
                                                    ...
                                                    <div id="{{ $p->id }}"
                                                        ...
                                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                            ...
                                                            <li>
                                                                <a href="{{ url('/pertumbuhan/ubah/' . $p->id) }}"
                        ...
                        {{ $pertumbuhan->links('vendor.pagination.tailwind') }}
                    </div>
                </div>
            </section>
        </div>
    ...
    <script>
        document.querySelectorAll('[data-modal-toggle="deleteModal"]').forEach(link => {
            link.addEventListener('click', (event) => {
                ...
                confirmDeleteForm.action = `/pertumbuhan/hapus/${id}`;
            });
        });
    </script>
