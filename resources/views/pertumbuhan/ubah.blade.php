            ...
            <section class="bg-gray-50 dark:bg-gray-900">
                <div class="mx-auto max-w-screen-xl">
                    <div
                        ...
                        <h1 class="text-sm font-medium dark:text-white">Pertumbuhan / Ubah</h1>
            ...
            <section class="bg-gray-50 dark:bg-gray-900">
                <div class="mx-auto max-w-screen-xl">
                    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-4">
                        <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Ubah Pertumbuhan</h2>
                        <form action="{{ url('pertumbuhan/prosesUbah/' . $pertumbuhan->id) }}" method="POST">
                            @csrf
                            {!! method_field('PUT') !!}
                            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                                <div class="w-full sm:col-span-2">
                                    ...
                                    <select id="nik" name="nik"
                                        ...
                                        <option value="" @if ($pertumbuhan->nik == '') selected @endif>Pilih
                                        </option>
                                        @foreach ($anak as $a)
                                            <option value="{{ $a->nik }}"
                                                @if ($pertumbuhan->nik == $a->nik) selected @endif>{{ $a->nik }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="w-full">
                                    <label for="berat_badan"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Berat Badan
                                        (kg)</label>
                                    <input type="text" name="berat_badan" id="berat_badan"
                                        ...
                                        placeholder="Berat Badan" value="{{ $pertumbuhan->berat_badan }}" required="">
                                </div>
                                <div class="w-full">
                                    <label for="tinggi_badan"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tinggi Badan
                                        (cm)</label>
                                    <input type="text" name="tinggi_badan" id="tinggi_badan"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-sky-600 focus:border-sky-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-sky-500 dark:focus:border-sky-500"
                                        placeholder="Tinggi Badan" value="{{ $pertumbuhan->tinggi_badan }}"
                                        required="">
                                </div>
                                <div class="w-full">
                                    <label for="umur"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Umur
                                        (bulan)</label>
                                    <input type="text" name="umur" id="umur"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-sky-600 focus:border-sky-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-sky-500 dark:focus:border-sky-500"
                                        placeholder="Umur" value="{{ $pertumbuhan->umur }}" required="">
                                </div>
                                <div class="w-full">
                                    <label for="tanggal_posyandu"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tanggal
                                        Posyandu</label>
                                    <div class="relative">
                                        <div
                                        ...
                                        <input datepicker datepicker-autohide datepicker-buttons datepicker-autoselect-today
                                            datepicker-format="dd/mm/yyyy" type="text" name="tanggal_posyandu"
                                            id="tanggal_posyandu"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            value="{{ $pertumbuhan->tanggal_posyandu }}" placeholder="DD/MM/YYYY">
                                    </div>
                                </div>
                            </div>
                            <button type="submit"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            function fetchAndFillData(nik) {
                if (nik) {
                    fetch(`/pertumbuhan/getPendudukByNik/${nik}`)
