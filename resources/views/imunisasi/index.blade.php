            ...
            <section class="bg-gray-50 dark:bg-gray-900">
                <div class="mx-auto max-w-screen-xl">
                    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-4">
                        <h1 class="text-sm font-medium dark:text-white">Imunisasi</h1>
                    </div>
                </div>
            </section>
        </div>
            ...
            <section class="bg-gray-50 dark:bg-gray-900">
                <div class="mx-auto max-w-screen-xl">
                    ...
                    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                        <div
                            ...
                            <div
                                ...
                                @if (Auth::user()->level == 'ADMIN' || Auth::user()->level == 'PETUGAS')
                                    <a href="{{ url('/imunisasi/tambah') }}" type="button"
                                    ...
                                    @foreach ($imunisasi as $i)
                                        <tr class="border-b dark:border-gray-700">
                                            <td scope="row"
                                                ...
                                                {{ $i->nama }}</td>
                                            <td class="px-4 py-3">{{ $i->nik }}</td>
                                            <td class="px-4 py-3">{{ $i->imunisasi }}</td>
                                            <td class="px-4 py-3">{{ $i->vitamin }}</td>
                                            <td class="px-4 py-3 format-tanggal">{{ $i->tanggal_imunisasi }}</td>
                                            @if (Auth::user()->level == 'ADMIN' || Auth::user()->level == 'PETUGAS')
                                                <td class="px-4 py-3 flex items-center justify-end">
                                                    <button id="apple-imac-27-dropdown-button"
                                                        data-dropdown-toggle="{{ $i->id }}"
                                                    ...
                                                    <div id="{{ $i->id }}"
                                                        ...
                                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                            ...
                                                            <li>
                                                                <a href="{{ url('/imunisasi/ubah/' . $i->id) }}"
                        ...
                        {{ $imunisasi->links('vendor.pagination.tailwind') }}
                    </div>
                </div>
            </section>
        </div>
    ...
    <script>
        document.querySelectorAll('[data-modal-toggle="deleteModal"]').forEach(link => {
            link.addEventListener('click', (event) => {
                ...
                confirmDeleteForm.action = `/imunisasi/hapus/${id}`;
            });
        });
    </script>
